...
main_keyboard = ReplyKeyboardMarkup(
    [["/add", "/search"], ["/list", "/stats"]], resize_keyboard=True
)
...
async def add_part_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if len(data) < 7:
            await update.message.reply_text(
                "❌ Недостаточно данных. Нужно минимум 7 строк! Попробуйте снова:"
            )
            return ADD_PART
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        total_value = (
            session.query(func.sum(db.AutoPart.price * db.AutoPart.quantity)).scalar()
            or 0
        )
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ADD_PART: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, add_part_process)
            ]
        },
